...
import cn.hzk.wms.req.CategoryREQ;
import cn.hzk.wms.service.ICategoryService;
import cn.hzk.entities.Category;
...
public class CategoryController {
    ...
    @PostMapping("/search") // 请求路径： localhost:8001/wms/category/search
    ...
    @GetMapping("/{id}") // localhost:8001/wms/category/{id}
    ...
    @GetMapping("/list") // 请求 localhost:8001/wms/category/list
    public Result list() {
        return categoryService.findAllNormal();
    }

    @ApiOperation("查询正常状态的分类及分类下的所有标签接口")
    @GetMapping("/label/list") // localhost:8001/wms/category/label/list
